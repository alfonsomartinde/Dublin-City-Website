	...
	<div class="container">
		...
		<div ng-repeat=" opp in newWhatsOn | filter: {date:  day.format('MMM Do YYYY') } | orderBy: 'date' as filtered_results" ng-animate="{enter: 'animate-enter', leave: 'animate-leave'}">
			...
			<div class="row item">
				<div class="col-md-4">
					<figure>
						<a ui-sref="whatsonDetails({slug:opp.opp.slug})" class="link-arrow-pink" title="{{opp.opp.title.rendered | to_trusted}}">See more</a>
						<img 
				...
				<div class="col-md-8">
					<a ui-sref="whatsonDetails({slug:opp.opp.slug})" title="{{opp.opp.title.rendered | to_trusted}}" class='title-link'><h2 ng-bind-html="opp.opp.title.rendered | to_trusted | limitTo: 300"></h2></a>
					<hr>
		...
		<div class="alert alert-info" ng-show="filtered_results.length == 0">
		   	 <h2 class="text-center">No opportunities on {{day.format('dddd, Do MMMM, YYYY')}}<br>Please select an underlined date</h2>
		</div>
